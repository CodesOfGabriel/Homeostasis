...
Cada evento interrompe a simulação e abre um período de análise. Você pode descer pelas escalas **Organismo → Tecido → Célula → Mitocôndria**, preparar sinais e recursos e escolher diretamente uma intervenção. Recuperação, resposta parcial ou falha são calculadas depois pelas métricas realmente observadas. Uma sinalização inadequada pode produzir hipoglicemia, alteração ventilatória, perda de perfusão, desequilíbrio hidroeletrolítico ou uma carga iatrogênica que continua agindo mesmo depois da decisão.
...
- escolha direta assim que os sinais e recursos exigidos pela intervenção estiverem preparados;
- debrief da resposta fisiológica, com valores iniciais, finais, metas homeostáticas e cadeia causal do caso;
...
- liberação hormonal e regulação central com efeitos, custos, riscos e tempo de ação, incluindo grelina, leptina, adiponectina, POMC/CART e NPY/AgRP;
...
- ingestão de água, barorreflexo, drive ventilatório bulbar, RAAS, ADH distal e autorregulação renal;
- domínio persistente em seis áreas, coleção de mecanismos, automações conquistadas e novos fenótipos por prestígio;
- exercício, estresse, nutrição, sono, infecção, calor, falência tireoidiana, hipertireoidismo e síndrome de Cushing como contextos que exigem adaptação;
- consequências persistentes e graduais para atrasos ou intervenções inadequadas.

As quatro views — **Organismo**, **Tecido**, **Célula** e **Mitocôndria** — compartilham o mesmo estado e continuam avançando na mesma timeline.
...
- `src/game/scenarios.ts`, `scenarioResolution.ts` e `scenarioLearning.ts`: mecanismos dos eventos, reservas, metas observáveis e avaliação da trajetória;
- `src/game/endocrine.ts`, `centralRegulation.ts` e `iatrogenic.ts`: sinalização hormonal, regulação central e consequências do uso inadequado;
- `src/game/acidBase.ts`, `cardiacRhythm.ts` e `mastery.ts`: interpretação clínica, substrato arrítmico e metaprogressão persistente;
